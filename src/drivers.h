...
const float offsetContactTime = 100;//320 Ct Default
...
int LOWBYTE=0;
    ...
    unsigned char incomingByte[9];
  unsigned char checkSum;
  Serial1.readBytes(incomingByte,8);
  Serial.print("OZONE:");
  Serial.println( (incomingByte[4]*256+incomingByte[5])*0.1 );
  for(int i=0;i<9;i++)
  {
  if(incomingByte[i]==255)
  HIGHBYTE = i+4;
  ...
  Serial.print("BYTE8:");
  Serial.println(incomingByte[8],HEX);
  //delay(1);
    
 // break;
 
LOWBYTE=HIGHBYTE+1;
if(HIGHBYTE==4 && LOWBYTE==5)
O3Value = (incomingByte[HIGHBYTE]*256+incomingByte[LOWBYTE])*0.1;

return (O3Value);
  /*
unsigned char incomingByte[9]={0};
byte flag = false;
unsigned char checkSum ='#'; 
while(Serial1.available()>0 && checkSum!=incomingByte[8])
{
Serial1.readBytes(incomingByte,8);
for(int i=1;i<=7;i++)
checkSum +=incomingByte[i];
...
Serial.println((incomingByte[3]*256+incomingByte[4])*0.1);
float ppm = (incomingByte[4]*256+incomingByte[5])*0.1;
if((ppm<=100 && ppm>=0))
return (incomingByte[4]*256+incomingByte[5])*0.1;
else
return 0;
...
for (int k = 0; k < O3numReadings; k++) {
  ...
  long sum = 0;                   // sum of samples taken
    ...
    for (int i = 0; i < 500; i++)
    {
        sum += analogRead(O3Pin);
        delayMicroseconds(1000);
    O3adcValue = sum / (float)500;
    //if(O3adcValue<300){output=0.0;}
  if(O3adcValue<496){O3Value=0.0;}
  else if(O3adcValue>2481){output=10.0;} 
  else if(496 < O3adcValue < 2481){ output = 10.0/2181.0*float(O3adcValue);//y=mx+c ==>y is ppm & x is adc; m=2/397=0.00503778,c=-2.4987.
  output = output-(1000.0/727.0);}// O3 sensor gives data from 0.4(496adc) to 2v(2481adc) equivalent to 0-10ppm.
  output = roundf(output * 100) / 100;
  if (output < 10)
  return output;
  else
  return 10.0f; 
